...
const translations = {
  'pt': {
    ...
    portfolio: {
      ...
      ecoterapia: 'Plataforma fullstack de automação de atendimento para consultas de terapia e emagrecimento via WhatsApp. O projeto implementa um sistema completo de agendamento e cobrança, integrando a Meta WhatsApp Business Cloud API para mensageria automatizada, AbacatePay para pagamentos PIX e Calendly para gestão de horários de consulta. O backend utiliza uma arquitetura robusta com máquina de estados para gerenciar o fluxo conversacional, filas de trabalho com Bull e Redis para lembretes automáticos, e MongoDB para persistência de dados.<br><br>O frontend foi desenvolvido em React com TypeScript e Vite, apresentando um design responsivo com Tailwind CSS. A aplicação oferece funcionalidades avançadas como webhooks de confirmação/cancelamento de agendamentos, sistema de lembretes 24h e 1h antes das consultas, painel administrativo Bull Board para monitoramento de filas, e coleta de métricas e analytics. Toda a infraestrutura é containerizada com Docker, incluindo configurações para desenvolvimento e produção, garantindo escalabilidade e fácil deploy.',
  ...
  'en': {
    ...
    portfolio: {
      ...
      ecoterapia: 'Fullstack platform for automated appointment scheduling for therapy and weight loss consultations via WhatsApp. The project implements a complete scheduling and billing system, integrating the Meta WhatsApp Business Cloud API for automated messaging, AbacatePay for PIX payments, and Calendly for appointment time management. The backend uses a robust architecture with a state machine to manage conversational flow, job queues with Bull and Redis for automatic reminders, and MongoDB for data persistence.<br><br>The frontend was developed in React with TypeScript and Vite, featuring a responsive design with Tailwind CSS. The application offers advanced features such as appointment confirmation/cancellation webhooks, reminder system 24h and 1h before appointments, Bull Board admin panel for queue monitoring, and metrics and analytics collection. The entire infrastructure is containerized with Docker, including configurations for development and production, ensuring scalability and easy deployment.',
...
function updateContent(lang) {
  ...
  const portfolioDescriptions = [t.portfolio.lucas, t.portfolio.ecoterapia, t.portfolio.paloma, t.portfolio.special, t.portfolio.orlandis, t.portfolio.mmk, t.portfolio.avenue];
  ...
  portfolioItems.forEach((item, index) => {
    ...
    if (staticSpan) {
      ...
      const isFullstack = staticSpan.textContent.includes('FULLSTACK');
      staticSpan.textContent = isReact ? t.portfolio.react : (isFullstack ? t.portfolio.fullstack : t.portfolio.static);
    }
